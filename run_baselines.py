...
def run_chaos_baseline(experiment_dir: str, round_num: int | str,
    ...
    """Evaluate a Chaos-1B checkpoint as a baseline."""
    ...
    if not os.path.exists(os.path.join(adv_adapter, "adapter_model.safetensors")):
        print(f"   Chaos-1B R{round_num}: no checkpoint found, skipping")
        return None

    import random
    print(f"\n--- Chaos-1B Round {round_num} ---")
    start = time.time()
    ...
    elapsed = time.time() - start
    asr = unsafe / len(attacks) if attacks else 0.0
    print(f"   Chaos-1B R{round_num}: {unsafe}/{len(attacks)} ({asr:.0%} ASR) in {elapsed:.0f}s")

    return {
        "method": f"Chaos-1B (R{round_num})",
        "num_attacks": len(attacks),
        "unsafe": unsafe,
        "asr": round(asr, 4),
        "elapsed_seconds": round(elapsed, 1),
...
def main():
    ...
    parser.add_argument("--experiment", type=str, default="experiments/default",
                        help="Chaos-1B experiment directory for checkpoint baselines")
    parser.add_argument("--chaos-rounds", nargs="*", type=int, default=[0, 9],
                        help="Chaos-1B checkpoint rounds to evaluate")
